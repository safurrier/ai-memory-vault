...
def normalize_url(url: str) -> str:
    ...
    - Substack app-link redirects (open.substack.com/pub/... and substack.com/app-link/...)
    - General query parameter cleanup
    """
    url = url.strip()

    # Resolve Substack app-link redirects
    # Pattern: open.substack.com/pub/<name>/p/<slug>?...
    # → https://<name>.substack.com/p/<slug>
    substack_open_match = re.match(
        r"https?://open\.substack\.com/pub/([^/]+)/p/([^?&#]+)", url
    )
    if substack_open_match:
        name, slug = substack_open_match.groups()
        return f"https://{name}.substack.com/p/{slug}"

    # Pattern: substack.com/app-link/post?publication_id=...&post_id=...
    # These can't be resolved without an API call, but we can at least
    # try extracting embedded URLs from the token parameter
    if "substack.com/app-link/" in url:
        parsed = urllib.parse.urlparse(url)
        params = urllib.parse.parse_qs(parsed.query)
        # Try to find publication_id and construct a clean URL
        # (Jina Reader will follow the redirect anyway)
        pass  # Let Jina Reader handle the redirect
    ...
    *,
    ...
    if json_mode:
        ...
        if "error" in warning.lower() or "404" in warning:
            ...
            result.title = inner.get("title", "")
            result.content = inner.get("content", "")
            return result

        result.title = inner.get("title", "")
        result.content = inner.get("content", "")
        result.source_url = inner.get("url", url)
        result.tokens = inner.get("usage", {}).get("tokens")
        result.metadata = {
