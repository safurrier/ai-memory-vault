# Obsidian AI Vault Instructions

Personal knowledge management vault using PARA methodology in Obsidian, with AI-assisted workflows via Claude Code skills.

## Agent Compatibility

`AGENTS.md` is the canonical instruction file. `CLAUDE.md` is a relative symlink to it so Claude Code discovers the same instructions; use Claude Code and `/personalize` as the primary beginner path. Git preserves symlinks on macOS and Linux. On platforms or archive tools that do not preserve them, recreate `CLAUDE.md` as a relative symlink to `AGENTS.md` rather than maintaining two copies.

Canonical skills live in `.agents/skills/`. `.claude/skills` is a relative compatibility symlink for Claude Code discovery. Add and edit skills only under `.agents/skills/`; recreate the compatibility symlink if a platform replaces it with a directory or text file.
...
`atomic` | `project` | `daily` | `moc` | `fleeting` | `literature` | `review` | `resource` | `area` | `archive` | `reference`
...
| `reference` | **Internal reference** — self-authored instructions, conventions, or lookup material | Relevant MOC or Home |
...
| `area` | **Ongoing responsibility** without a fixed completion date | Areas |
| `archive` | **Completed or inactive material** retained for context | Archives |
| `daily` | **Date-scoped journal or log** | Relevant MOC when useful |
| `fleeting` | **Temporary capture** awaiting review or organization | Staging or relevant MOC |
...
| Plugin | Purpose |
|--------|---------|
| obsidian-git | Optional Git-based backup and sync; configure only after verifying the intended private remote |
...
Git sync is optional. Enable the obsidian-git plugin only after verifying that `origin` is the intended private repository and reviewing its commit, pull, and push settings. If Git sync is enabled, follow these rules to avoid merge conflicts and history noise:
